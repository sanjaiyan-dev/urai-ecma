...
export const UserProfile: React.FC<UserProfileProps> = ({ userId, isActive = true })=>{
    ...
    if (loading) {
        return <div className="/* UI: Centered content in a dark background card with visible borders and shadow. */">Loading profile...</div>;
    }
    return (<div className={`p-6 rounded-2xl ${isActive ? 'bg-indigo-900 border-indigo-500' : 'bg-gray-800 border-gray-600'} flex flex-col gap-4 shadow-2xl`}>
            ...
            <p className="/* UI: Small size gray text color */">User ID: {userId}</p>
            <button onClick={handleRefresh} className="/* UI: Medium rectangular button with blue background, hover effect, and smooth transition. */">
                Refresh Profile
            </button>
        </div>);
};
...
**Function Summaries**:
- Line 7: `calculateDiscountPrice` -> *Calculates the discounted price of an item by applying a percentage reduction to the specified original cost while ensuring the resulting price is not negative.*
- Line 26: `calculateincreaseAmt` -> *Calculates the discounted price of an item by applying a given percentage reduction to the original cost while ensuring the resulting price is non-negative.*
...
export function calculateincreaseAmt(originalPrice: number, discountPercentage: number): number {
    if (originalPrice <= 0) {
        return 0;
    }
    if (discountPercentage <= 0) {
